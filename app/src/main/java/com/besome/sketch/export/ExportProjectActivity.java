...
import static mod.SketchProXUtil.getDip;
...
import com.SketchProX.remod.BuildConfig;
...
public class ExportProjectActivity extends BaseAppCompatActivity {
    ...
    /**
     * /SketchProX/signed_apk
     */
    private String signed_apk_postfix;
    /**
     * /SketchProX/export_src
     */
    private String export_src_postfix;
    /**
     * /sdcard/SketchProX/export_src
     */
    ...
    private void f(String filePath) {
        ...
        SketchProXUtil.toast(Helper.getResString(R.string.sign_apk_title_export_apk_file));
        ...
        try {
            ...
            runOnUiThread(() -> {
                Log.e("ProjectExporter", "While trying to export project's sources: "
                        + e.getMessage(), e);
                SketchProXUtil.showAnErrorOccurredDialog(this, Log.getStackTraceString(e));
    ...
    private void initializeAppBundleExportViews() {
        ...
        btnExportAppBundle.setOnClickListener(v -> {
            ...
            if (BuildConfig.FLAVOR.equals(BuildConfig.FLAVOR_NAME_WITHOUT_AABS)) {
                ...
                dialog.a("This SketchProX version doesn't support building AABs as it must work on " +
                        "Android 7.1.1 and earlier. Use SketchProX " + BuildConfig.VERSION_NAME_WITHOUT_FLAVOR + "-" +
                ...
                GetKeyStoreCredentialsDialog credentialsDialog = new GetKeyStoreCredentialsDialog(this,
                        ...
                        "Copy your keystore to /Internal storage/SketchProX/keystore/release_key.jks " +
        ...
        btn_sign_apk.setOnClickListener(v -> {
            GetKeyStoreCredentialsDialog credentialsDialog = new GetKeyStoreCredentialsDialog(this,
                    ...
                    "To sign an APK, you need a keystore. Use your already created one, and copy it to " +
                            "/Internal storage/SketchProX/keystore/release_key.jks and enter the alias's password.\n" +
    ...
    private void initializeOutputDirectories() {
        signed_apk_postfix = File.separator + "SketchProX" + File.separator + "signed_apk";
        export_src_postfix = File.separator + "SketchProX" + File.separator + "export_src";
        /* /sdcard/SketchProX/signed_apk */
    ...
    private static class BuildingAsyncTask extends MA implements DialogInterface.OnCancelListener, BuildProgressReceiver {
        ...
        public void b() {
            ...
            try {
                ...
                oB oBVar = new oB();
                /* Check if /Internal storage/SketchProX/signed_apk/ exists */
                if (!oBVar.e(wq.o())) {
                ...
                if (throwable instanceof LoadKeystoreException &&
                    ...
                    activity.get().runOnUiThread(() -> SketchProXUtil.showAnErrorOccurredDialog(activity.get(),
        ...
        public void a() {
            ...
            if (buildingAppBundle && new File(Environment.getExternalStorageDirectory(),
                    "SketchProX" + File.separator + "signed_aab" + File.separator + aabFilename).exists()) {
                ...
                dialog.a("You can find the generated, signed AAB file at:\n" +
                        "/Internal storage/SketchProX/signed_aab/" + aabFilename);
        ...
        public void a(String str) {
            ...
            // Dismiss the ProgressDialog
            activity.get().i();
            SketchProXUtil.showAnErrorOccurredDialog(activity.get(), str);
            activity.get().layout_apk_path.setVisibility(View.GONE);
            LottieAnimationView loading_sign_apk = this.loading_sign_apk.get();
            if (loading_sign_apk.isAnimating()) {
                loading_sign_apk.cancelAnimation();
