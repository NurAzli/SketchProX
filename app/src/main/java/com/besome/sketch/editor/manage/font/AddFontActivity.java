...
public class AddFontActivity extends BaseDialogActivity implements View.OnClickListener {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_FONT_PICKER && resultCode == RESULT_OK) {
            ...
            String filenameExtension = FileUtil.getFileExtension(SketchProXUtil.getSafDocumentDisplayName(intentData).orElse(".ttf"));
            SketchProXUtil.copySafDocumentToTempFile(intentData, this, filenameExtension, tempFontFile -> {
                ...
                try {
                    ...
                    if (typeface.equals(Typeface.DEFAULT)) {
                        SketchProXUtil.toastError("Warning: Font doesn't seem to be valid");
                    }
                    ...
                    SketchProXUtil.toast("Couldn't load font: " + e.getMessage());
                    LogUtil.e("AddFontActivity", "Failed to load font", e);
                }
            }, e -> {
                SketchProXUtil.toastError("Error while loading font: " + e.getMessage());
                LogUtil.e("AddFontActivity", "Failed to load font", e);
            });
        }
    }
