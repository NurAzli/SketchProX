...
public class AddSoundActivity extends BaseDialogActivity implements View.OnClickListener {
    ...
    private void saveSound() {
        if (isSoundValid(soundNameValidator)) {
            ...
            SketchProXUtil.copySafDocumentToTempFile(soundUri, this, FileUtil.getFileExtension(getFilenameOfPickedFile(soundUri)),
                    tempFile -> {
                        ...
                        SketchProXUtil.toastError("Error while loading sound: " + e.getMessage());
    ...
    private String getFilenameOfPickedFile(Uri input) {
        Optional<String> displayName = SketchProXUtil.getSafDocumentDisplayName(input);
        ...
        if (name.contains(".")) {
            ...
            Optional<String> optionalFilenameExtension = SketchProXUtil.doSingleStringContentQuery(input, MediaStore.MediaColumns.MIME_TYPE);
