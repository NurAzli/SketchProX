...
public class ManageLocalLibraryActivity extends Activity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        linear.setOnClickListener(v1 -> {
            ...
            if (url.isEmpty()) {
                SketchProXUtil.toastError("Please enter a dependency");
                return;
            }

            var parts = url.split(":");
            if (parts.length != 3) {
                SketchProXUtil.toastError("Invalid dependency format");
                return;
            }
            var group = parts[0];
            ...
            Executors.newSingleThreadExecutor().execute(() -> {
                ...
                resolver.resolveDependency(new DependencyResolver.DependencyResolverCallback() {
                    ...
                    public void dexingFailed(@NonNull String dependency, @NonNull Exception e) {
                        handler.post(() -> {
                            dialog.dismiss();
                            SketchProXUtil.showAnErrorOccurredDialog(ManageLocalLibraryActivity.this,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().hasExtra("sc_id")) {
            ...
            local_lib_file = FileUtil.getExternalStorageDir().concat("/.SketchProX/data/").concat(sc_id.concat("/local_library"));
        }
        local_libs_path = FileUtil.getExternalStorageDir().concat("/.SketchProX/libs/local_libs/");
        loadFiles();
    }
