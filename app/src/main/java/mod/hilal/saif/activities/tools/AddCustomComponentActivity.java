...
import com.SketchProX.remod.databinding.ManageCustomComponentAddBinding;
...
public class AddCustomComponentActivity extends AppCompatActivity implements View.OnClickListener {
        ...
        if (id == R.id.btn_save) {
            if (!isImportantFieldsEmpty()) {
                if (OldResourceIdMapper.isValidIconId(binding.componentIcon.getText().toString())) {
                    ...
                    SketchProXUtil.toastError(Helper.getResString(R.string.invalid_icon_id));
                    binding.componentIcon.requestFocus();
                }
            } else {
                SketchProXUtil.toastError(Helper.getResString(R.string.invalid_required_fields));
            }
    ...
    private void save() {
        ...
        SketchProXUtil.toast(Helper.getResString(R.string.common_word_saved));
        finish();
    }
        ...
        SketchFilePickerDialog dialog = new SketchFilePickerDialog(this)
                ...
                .setOnFileSelectedListener((SketchFilePickerDialog _dialog, File file) -> {
                    try {
                        ...
                        SketchProXUtil.toastError(Helper.getResString(R.string.publish_message_dialog_invalid_json));
                    }
                    _dialog.dismiss();
                });
    ...
    private void selectComponentToImport(String path) {
        ...
        if (readResult.first.isPresent()) {
            SketchProXUtil.toastError(readResult.first.get());
            return;
        }
        ...
        if (componentNames.size() > 1) {
            ...
            dialog.b(Helper.getResString(R.string.common_word_import), view -> {
                ...
                if (position != -1 && ComponentsHandler.isValidComponent(component)) {
                    ...
                    SketchProXUtil.toastError(Helper.getResString(R.string.invalid_component));
                }
                dialog.dismiss();
            });
