...
public class BlockLoader {
    ...
    public static ExtraBlockInfo getBlockFromProject(String sc_id, String block_name) {
        File customBlocksConfig = new File(Environment.getExternalStorageDirectory(),
                ".SketchProX/data/" + sc_id + "/custom_blocks");
        if (customBlocksConfig.exists()) {
            try {
                ...
                SketchProXUtil.toastError("Failed to get Custom Blocks for project " + sc_id + ": " + e.getMessage());
            }
        }
    ...
    private static void loadCustomBlocks() {
        ...
        for (int i = 0; i < arrList.size(); i++) {
            ...
            if (name instanceof String) {
                ...
                info.setName("");
                SketchProXUtil.toastError("Invalid name entry in Custom Block #" + (i + 1));
                continue;
            }
            ...
            if (map.containsKey("color")) {
                ...
                if (color instanceof String) {
                    try {
                        ...
                        SketchProXUtil.toastError("Invalid color in Custom Block #" + (i + 1));
                        continue;
                    }
                }
            } else {
                                    ...
                                    if (mapPaletteNumber == indexInt) {
                                        ...
                                        if (paletteColor instanceof Integer) {
                                            try {
                                                ...
                                                SketchProXUtil.toastError("Invalid color in Custom Block palette #" + (j + 1));
                                            }
                                        } else {
                                            SketchProXUtil.toastError("Invalid color value type in Custom Block palette #" + (j + 1));
                                        }
                                    }
                                } else {
                                    SketchProXUtil.toastError("Invalid palette index value type in Custom Block palette #" + (j + 1));
                                }
                            }
                        } catch (NumberFormatException e) {
                            SketchProXUtil.toastError("Invalid palette number in Custom Block #" + (i + 1));
                            continue;
                        }
                    } else {
                        SketchProXUtil.toastError("Invalid palette number value type in Custom Block #" + (i + 1));
                        continue;
                    }
                }
            }
